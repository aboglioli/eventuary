...
Appends fsync on the `SyncPolicy` schedule, which defaults to once per megabyte; choose `SyncPolicy::Always` when no acknowledged event may be lost. Checkpoint and coordinator state is written through a temp file, rename, and directory fsync, so a surviving file is never a partial one. Retention is caller-driven: `FsWriter::enforce_retention` deletes segments a `RetentionPolicy` has aged or sized out, and nothing calls it for you. A consumer whose checkpoint falls behind the retained range gets an error rather than a silent skip.
